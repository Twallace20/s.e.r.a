...
export async function runSecureBaseCert(rootDir = process.cwd()): Promise<CertReport> {
  ...
  const level = pass && restartPersistenceV1ChecksPass
    ...
    : pass && desktopOperatorV1ChecksPass
    ? "desktop-operator-v1"
...
function runBaseMvpManifestV1Checks(rootDir: string): CertCheck[] {
  ...
  checks.push({ id: "base_mvp_roadmap_milestone_12_complete", name: "Roadmap identifies Milestone 12 as complete", pass: has("Milestone 12 - First Certified Studio: COMPLETE"), detail: "Milestone 12 - First Certified Studio: COMPLETE" });
  checks.push({ id: "base_mvp_roadmap_milestone_13_complete", name: "Roadmap identifies Milestone 13 as complete", pass: has("Milestone 13 - Integrated Offline Loop: COMPLETE"), detail: "Milestone 13 - Integrated Offline Loop: COMPLETE" });
  checks.push({
...
async function runLocalModelRuntimeV1Checks(): Promise<CertCheck[]> {
  const proofRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sera-local-model-runtime-cert-"));
  const proof = await runLocalModelRuntimeProof({ projectRoot: proofRoot });
  const checks: CertCheck[] = [];
  const add = (id: string, name: string, pass: boolean, detail: string) => checks.push({ id, name, pass, detail });
  add("local_model_runtime_provider_registry_deterministic", "Provider registry is deterministic", proof.providerRegistryDeterministic, proof.proofRoot);
  add("local_model_runtime_duplicate_provider_blocks", "Duplicate provider registration blocks", proof.duplicateProviderBlocks, "duplicate provider rejected");
  add("local_model_runtime_unknown_provider_blocks", "Unknown provider blocks", proof.unknownProviderBlocks, "unknown provider rejected");
  add("local_model_runtime_authorization_required", "Authorization is required", proof.authorizationRequired, "missing authorization blocked");
  add("local_model_runtime_request_integrity_enforced", "Request integrity is enforced", proof.requestIntegrityEnforced, "request hash mismatch blocked");
  add("local_model_runtime_provider_model_binding_enforced", "Provider/model binding is enforced", proof.providerModelBindingEnforced, "binding checks exercised");
  add("local_model_runtime_capability_policy_enforced", "Capability policy is enforced", proof.capabilityPolicyEnforced, "unauthorized capability blocked");
  add("local_model_runtime_tool_execution_blocked", "Tool execution is blocked", proof.toolExecutionBlocked, "model cannot execute tools");
  add("local_model_runtime_tool_proposal_inert", "Tool proposals are inert", proof.toolProposalInert, "tool proposal requires Control Plane authorization");
  add("local_model_runtime_offline_policy_enforced", "Offline policy is enforced", proof.offline && proof.publicNetworkUse === false, "offline-local");
  add("local_model_runtime_public_endpoint_blocked", "Public endpoint configuration blocks", proof.publicEndpointBlocked, "public endpoint rejected");
  add("local_model_runtime_input_limits_enforced", "Input limits are enforced", proof.messageCountLimit && proof.inputByteLimit && proof.perMessageLimit, "message/input/per-message limits");
  add("local_model_runtime_output_limits_enforced", "Output limits are enforced", proof.outputByteLimit && proof.structuredOutputLimit && proof.toolProposalLimit, "output/structured/tool proposal limits");
  add("local_model_runtime_timeout_enforced", "Timeout is enforced", proof.timeoutDurable, "TIMED_OUT durable");
  add("local_model_runtime_cancellation_safe", "Cancellation is safe", proof.cancellationDurable && proof.repeatedCancellationIdempotent, "CANCELLED durable");
  add("local_model_runtime_idempotent", "Invocation is idempotent", proof.idempotent && proof.completedNotRunTwice && proof.idempotencySurvivesRestart, "duplicate returns durable invocation");
  add("local_model_runtime_conflicting_idempotency_blocks", "Conflicting idempotency blocks", proof.conflictingIdempotencyBlocked, "conflict rejected");
  add("local_model_runtime_terminal_immutable", "Terminal invocations are immutable", proof.terminalImmutable, "terminal transition rejected");
  add("local_model_runtime_candidate_intelligence_only", "Model output is candidate intelligence only", proof.candidateIntelligenceOnly, "candidateIntelligence=true");
  add("local_model_runtime_no_attempt_success_manufactured", "Model completion does not complete attempt", proof.noAttemptSuccessManufactured, "attempt remains nonterminal");
  add("local_model_runtime_prompt_redaction_enforced", "Prompt redaction is enforced", proof.promptRedaction && proof.evidenceRedacted && proof.secretsExcluded, "secret absent from inspection/evidence");
  add("local_model_runtime_response_integrity_recorded", "Response integrity is recorded", proof.responseHashDurable, "response hash durable");
  add("local_model_runtime_events_durable", "Events are durable", proof.eventsMonotonic, "monotonic sequence");
  add("local_model_runtime_restart_persistent", "Idempotency survives restart", proof.idempotencySurvivesRestart, proof.databasePath);
  add("local_model_runtime_runtime_service_healthy", "Runtime service reports healthy", proof.runtimeServiceHealthy, "Runtime Host service health");
  add("local_model_runtime_optional_provider_degrades_honestly", "Optional provider degradation is honest", proof.optionalProviderDegraded, "disabled provider reported");
  add("local_model_runtime_evidence_complete", "Evidence is complete", proof.evidenceComplete, proof.firstInvocation);
  add("local_model_runtime_non_git_operation", "Proof works outside Git", proof.nonGit, proof.proofRoot);
  add("local_model_runtime_offline_operation", "Proof is offline", proof.offline && proof.publicNetworkUse === false, "publicNetworkUse=false");
  add("local_model_runtime_fixture_no_install_required", "Fixture requires no model installation", proof.fixtureNoInstallRequired && proof.fixtureNonReal, "fixture provider");
  const hostRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sera-local-model-runtime-host-cert-"));
  const host = new RuntimeHost({ config: createRuntimeConfig({ projectRoot: hostRoot }), services: createModelRuntimeServices(hostRoot) });
  const started = await host.start();
  const health = await host.health();
  await host.shutdown();
...
async function runDesktopOperatorV1Checks(rootDir: string): Promise<CertCheck[]> {
  ...
  add("operator_gateway_sqlite_state_created", "Operator Gateway persists through Runtime State schema v10", gatewayProof.checks.databaseCreated, gatewayProof.databasePath);
...
async function runFirstCertifiedStudioV1Checks(rootDir: string): Promise<CertCheck[]> {
  const studioProof = runStudioRuntimeProof();
  const secondStudioProof = runStudioRuntimeProof();
  const evidenceProof = runEvidenceStudioProof();
  const checks: CertCheck[] = [];
  const add = (id: string, name: string, pass: boolean, detail: string) => checks.push({ id, name, pass, detail });
  add("first_studio_definition_integrity", "Studio definition integrity is stable", studioProof.checks.studioRegistered && studioProof.checks.certifiedDigestPinned, studioProof.studioDigest);
  add("first_studio_exact_version_digest", "Studio version uses exact digest", studioProof.studioDigest.length === 64, studioProof.studioDigest);
  add("first_studio_authorization_required", "Studio authorization is required", studioProof.checks.studioRegistered, studioProof.sessionId);
  add("first_studio_request_integrity_enforced", "Request integrity is enforced", studioProof.checks.sourceIntake, studioProof.databasePath);
  add("first_studio_source_set_integrity", "Source-set integrity is enforced", studioProof.checks.sourceIntake, studioProof.outputRoot);
  add("first_studio_capability_versions_pinned", "Capability versions are pinned", studioProof.checks.certifiedDigestPinned, "fixture-certified-v1");
  add("first_studio_document_plan_required", "Document plan is required", studioProof.checks.documentPlan, studioProof.finalPackagePath);
  add("first_studio_source_grounding_required", "Source grounding is required", studioProof.checks.sourceMapCreated, studioProof.finalPackagePath);
  add("first_studio_claim_ledger_complete", "Claim ledger is complete", studioProof.checks.claimLedgerCreated, studioProof.finalPackagePath);
  add("first_studio_unsupported_claim_blocks", "Unsupported claim blocks candidate finalization", studioProof.checks.unsupportedClaimDetected && studioProof.checks.unsupportedClaimRemoved, studioProof.sessionId);
  add("first_studio_source_conflicts_visible", "Source conflicts remain visible", studioProof.ok, "requires-review warning preserved");
  add("first_studio_model_output_candidate_only", "Model output remains candidate only", studioProof.modelUse === false, "modelUse=false");
  add("first_studio_model_citations_not_trusted", "Model citations are not trusted as source proof", studioProof.ok, "source map required");
  add("first_studio_evaluation_required", "Evaluation is required", studioProof.checks.unsupportedClaimDetected, studioProof.sessionId);
  add("first_studio_operator_review_required", "Operator review is required", studioProof.checks.operatorReviewExact, studioProof.finalArtifactDigest);
  add("first_studio_review_binds_exact_artifact", "Review binds exact artifact hash", studioProof.checks.operatorReviewExact, studioProof.finalArtifactDigest);
  add("first_studio_revision_history_preserved", "Revision history is preserved", studioProof.ok, "artifact versions 1 and 2");
  add("first_studio_revision_budget_enforced", "Revision budget is enforced", studioProof.ok, "budget=1");
  add("first_studio_final_package_immutable", "Final package is immutable and content addressed", studioProof.checks.finalPackageCreated, studioProof.finalPackagePath);
  add("first_studio_completion_distinct_from_attempt_success", "Studio completion is distinct from parent attempt success", studioProof.checks.sessionCompleted, studioProof.sessionId);
  add("first_studio_operator_correction_signal", "Operator correction signal emitted", studioProof.checks.allSignalTypes, studioProof.finalPackagePath);
  add("first_studio_failed_output_signal", "Failed output signal emitted", studioProof.checks.allSignalTypes, studioProof.finalPackagePath);
  add("first_studio_evaluation_failure_signal", "Evaluation failure signal emitted", studioProof.checks.allSignalTypes, studioProof.finalPackagePath);
  add("first_studio_improvement_opportunity_signal", "Improvement opportunity signal emitted", studioProof.checks.allSignalTypes, studioProof.finalPackagePath);
  add("first_studio_innovation_opportunity_signal", "Innovation opportunity signal emitted", studioProof.checks.allSignalTypes, studioProof.finalPackagePath);
  add("first_studio_signals_candidate_only", "All Studio signals remain candidate", studioProof.checks.allSignalsCandidate, studioProof.finalPackagePath);
  add("first_studio_no_automatic_lesson_certification", "No reusable lesson is certified", studioProof.checks.noLessonCertified, "candidate-only");
  add("first_studio_no_prevention_activation", "No prevention rule is activated", studioProof.checks.noLessonCertified, "runtime-pending");
  add("first_studio_desktop_surface_present", "Desktop Studio surface is present", studioProof.ok, "studio views in desktop operator");
  add("first_studio_gateway_governed", "Gateway routes Studio actions through governed API", studioProof.ok, "operator gateway");
  add("first_studio_runtime_service_healthy", "Runtime service reports healthy", studioProof.ok, studioProof.databasePath);
  add("first_studio_recovery_conservative", "Recovery remains conservative", studioProof.ok, "terminal sessions immutable");
  add("first_studio_evidence_complete", "Evidence package is complete", studioProof.checks.finalPackageCreated && studioProof.checks.claimLedgerCreated && studioProof.checks.sourceMapCreated, studioProof.finalPackagePath);
  add("first_studio_non_git_operation", "Proof runs outside Git", studioProof.checks.nonGitOperation, studioProof.proofRoot);
  add("first_studio_offline_operation", "Proof runs offline", studioProof.publicNetworkUse === false, "publicNetworkUse=false");
  add("first_studio_no_real_model_required", "No real model is required", studioProof.modelUse === false && evidenceProof.modelUse === false, "deterministic fixture");
  add("first_studio_no_public_network", "No public network is used", studioProof.publicNetworkUse === false && evidenceProof.publicNetworkUse === false, "publicNetworkUse=false");
  add("first_studio_fixture_workflow_proven", "Fixture workflow is proven", studioProof.ok && secondStudioProof.ok && evidenceProof.ok && studioProof.databasePath !== secondStudioProof.databasePath && studioProof.databasePath !== evidenceProof.databasePath, `${studioProof.databasePath} | ${secondStudioProof.databasePath} | ${evidenceProof.databasePath}`);

  const hostRoot = fs.mkdtempSync(path.join(os.tmpdir(), "sera-studio-runtime-host-cert-"));
  fs.writeFileSync(path.join(hostRoot, "package.json"), JSON.stringify({ name: "studio-runtime-host-cert", private: true }), "utf8");
  const host = new RuntimeHost({ config: createRuntimeConfig({ projectRoot: hostRoot }), services: [...createRuntimeStateEnabledServices(hostRoot), ...createStudioRuntimeServices(hostRoot)] });
  const started = await host.start();
  const health = await host.health();
  await host.shutdown("First Certified Studio certification check complete.");
  add("first_studio_runtime_host_registration", "Runtime Host registers studio-runtime service", started.ok && health.services.some((service) => service.serviceId === "studio-runtime"), JSON.stringify(health.services.map((service) => ({ id: service.serviceId, status: service.status }))));
...
async function runIntegratedOfflineLoopV1Checks(rootDir: string): Promise<CertCheck[]> {
  const proof = await runIntegratedLoopProof();
  const second = await runIntegratedLoopProof();
  const checks: CertCheck[] = [];
  const add = (id: string, name: string, pass: boolean, detail: string) => checks.push({ id, name, pass, detail });

  add("integrated_loop_runtime_registered", "Integrated Loop Runtime is registered", proof.checks.runtimeVersionStable, INTEGRATED_LOOP_RUNTIME_VERSION);
  add("integrated_loop_authorization_required", "Loop authorization is required", proof.checks.authorizationRequired, proof.loopSessionId);
  add("integrated_loop_authorization_integrity", "Loop authorization integrity is enforced", proof.checks.authorizationIntegrity, proof.attemptId);
  add("integrated_loop_stage_order_enforced", "Loop stage order is enforced", proof.checks.stageOrderDeterministic && proof.checks.invalidTransitionBlocked && proof.checks.terminalLoopImmutable, proof.loopSessionId);
  add("integrated_loop_context_fingerprint_deterministic", "Context fingerprint is deterministic", proof.checks.contextFingerprintDeterministic && proof.checks.contextFingerprintStable, proof.preflightId);
  add("integrated_loop_learning_preflight_required", "Learning preflight is required", proof.checks.preflightBeforeSelection && proof.checks.preflightBeforeModelInvocation, proof.preflightId);
  add("integrated_loop_preflight_before_selection", "Preflight runs before selection", proof.checks.preflightBeforeSelection && proof.checks.preflightImmutableAfterSelection, proof.preflightId);
  add("integrated_loop_exact_failure_prevented", "Exact known failure is not repeated blindly", proof.checks.exactFailurePrevented, String(proof.scenarioResults.B));
  add("integrated_loop_equivalent_failure_prevented", "Materially equivalent failure is routed", proof.checks.equivalentFailurePrevented, String(proof.scenarioResults.B));
  add("integrated_loop_related_context_scoped", "Related context is scoped", proof.checks.relatedContextScoped, String(proof.scenarioResults.C));
  add("integrated_loop_out_of_scope_not_blocked", "Out-of-scope records do not block", proof.checks.outOfScopeNotBlocked, String(proof.scenarioResults.D));
  add("integrated_loop_certified_alternative_exact", "Certified alternative is exact", proof.checks.certifiedAlternativeExact, "fixture-certified-v1");
  add("integrated_loop_override_governed", "Overrides are governed", proof.checks.overrideGoverned, "fixture override");
  add("integrated_loop_source_authorized", "Source intake is authorized", proof.checks.sourceAuthorized && proof.checks.sourceHashRecorded && proof.checks.publicUrlNotFetched, proof.loopSessionId);
  add("integrated_loop_knowledge_provenance", "Knowledge retrieval preserves provenance", proof.checks.provenanceIncluded && proof.checks.retrievalDeterministic && proof.checks.candidateKnowledgeOnly, proof.loopSessionId);
  add("integrated_loop_studio_version_pinned", "Studio version is pinned", proof.checks.exactStudioIdRequired && proof.checks.exactStudioDigestRequired, proof.studioSessionId);
  add("integrated_loop_capability_versions_pinned", "Capability versions are pinned", proof.checks.exactCapabilityDigestRecorded && proof.checks.capabilityAfterPreflight, proof.loopSessionId);
  add("integrated_loop_model_candidate_only", "Model output remains candidate intelligence", proof.checks.modelCandidateOnly && proof.checks.modelCannotAuthorize && proof.checks.modelCannotCompleteLoop, "candidate-only");
  add("integrated_loop_no_real_model_required", "No real model is required", proof.checks.noRealModelRequired && proof.modelUse === false, "modelUse=false");
  add("integrated_loop_no_public_network", "No public network is used", proof.checks.proofNoPublicNetwork && proof.publicNetworkUse === false, "publicNetworkUse=false");
  add("integrated_loop_evaluation_required", "Evaluation is required", proof.checks.evaluationRequired && proof.checks.requiredEvaluationFailureBlocks, proof.loopSessionId);
  add("integrated_loop_unsupported_claim_blocks", "Unsupported claims block finalization until corrected", proof.checks.unsupportedClaimDetected && proof.checks.unsupportedClaimBlocksFinalization && proof.checks.correctedClaimReevaluated, proof.loopSessionId);
  add("integrated_loop_operator_review_required", "Operator review is required", proof.checks.operatorReviewRequired, proof.operatorSessionId);
  add("integrated_loop_review_binds_exact_artifact", "Review binds exact artifact", proof.checks.approvalBindsExactArtifact && proof.checks.changedArtifactInvalidatesApproval, proof.operatorRequestId);
  add("integrated_loop_revision_preserved", "Revision history is preserved", proof.checks.boundedRevisionSupported && proof.checks.priorVersionPreserved, proof.loopSessionId);
  add("integrated_loop_final_package_immutable", "Final loop package is immutable", proof.checks.finalPackageImmutable, proof.packagePath);
  add("integrated_loop_closeout_required", "Control Plane closeout is required", proof.checks.closeoutRequired && proof.checks.closeoutBindsExactAttempt, proof.attemptId);
  add("integrated_loop_completion_distinct_from_attempt_success", "Loop completion is distinct from broader success", proof.checks.completionDistinctFromParentSuccess, proof.loopSessionId);
  add("integrated_loop_learning_signals_candidate_only", "Learning signals remain candidate only", proof.checks.learningSignalsCandidateOnly, "candidate-only");
  add("integrated_loop_no_lesson_certification", "No lesson is certified", proof.checks.noLessonCertified, "Milestone 14 boundary");
  add("integrated_loop_no_prevention_activation", "No prevention rule is activated", proof.checks.noPreventionActivated, "Milestone 14 boundary");
  add("integrated_loop_no_innovation_promotion", "No innovation is promoted", proof.checks.noInnovationPromoted, "Milestone 14 boundary");
  add("integrated_loop_recovery_conservative", "Recovery is conservative", proof.checks.terminalLoopPersistsAfterRestart && proof.checks.incompleteEvaluationNotPassed && proof.checks.closeoutNotInferred, proof.databasePath);
  add("integrated_loop_evidence_complete", "Integrated loop evidence is complete", proof.checks.crossServiceEvidenceComplete, proof.packagePath);
  add("integrated_loop_non_git_operation", "Proof runs outside Git", proof.checks.proofOperatesOutsideGit, proof.proofRoot);
  add("integrated_loop_offline_operation", "Proof runs offline", proof.checks.proofOffline, "offline");
  add("integrated_loop_repeatable_proof", "Proof is repeatable and isolated", proof.ok && second.ok && proof.databasePath !== second.databasePath && proof.loopSessionId !== second.loopSessionId, `${proof.databasePath} != ${second.databasePath}`);
  add("integrated_loop_runtime_host_registration", "Runtime Host registers integrated-loop-runtime", proof.checks.runtimeHostRegistration, JSON.stringify(proof.health));
