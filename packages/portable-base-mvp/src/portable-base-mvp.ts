...
import { verifyRestrictedUserEvidence } from "./restricted-user-proof";
...
const claims: CapabilityClaim[] = [
  ...
  {
    claimId: "same-host-restricted-user-release-independence",
    layer: "Desktop",
    statement: "A same-host restricted standard-user profile can prove release independence and one certified host profile using release-only execution.",
    proofRequired: ["restricted-user-token", "source-denial-probes", "bundled-runtime", "network-denial", "desktop-visual-proof", "restart-recovery"],
    status: "blocked",
    authorityBoundary: "Successful real evidence may grant RELEASE_INDEPENDENCE_PROVEN and CERTIFIED_HOST_PROFILE_PROVEN only; it never grants CROSS_HOST_PORTABILITY_PROVEN."
  }
];
...
export function buildPortableBaseMvp(input: { projectRoot?: string; outputRoot?: string } = {}): BaseMvpReleaseResult {
  ...
  const manifest = {
    ...
    collectors: {
      entrypoint: collectorManifestRecord(packageRoot, "collectors/restricted-user-collector.cjs"),
...
export async function runPortableBaseMvpProof(input: { projectRoot?: string; cleanEnvironmentEvidenceRoot?: string; restrictedUserEvidenceRoot?: string; modelId?: string } = {}): Promise<BaseMvpProofResult> {
  ...
  const restrictedEvidenceValidation = input.restrictedUserEvidenceRoot ? verifyRestrictedUserEvidence(input.restrictedUserEvidenceRoot) : undefined;
  const cleanEnvironmentProvided = (cleanEvidenceValidation.ok && !cleanEvidenceValidation.fixtureOnly && cleanEvidenceValidation.status === "VALID_EXTERNAL_EVIDENCE") || restrictedEvidenceValidation?.status === "RESTRICTED_PROOF_VERIFIED";
  const checks = {
    ...
    restrictedUserEvidenceVerified: restrictedEvidenceValidation?.ok ?? false,
...
function writePortableCollectors(packageRoot: string): void {
  const target = path.join(packageRoot, "collectors");
  fs.mkdirSync(target, { recursive: true });
  const compiledRoot = fs.existsSync(path.join(__dirname,"restricted-user-proof.js"))?__dirname:path.resolve(__dirname,"..","dist");
  for (const name of fs.readdirSync(compiledRoot).filter((name) => /^restricted-user-.*\.js$/.test(name)).sort()) fs.copyFileSync(path.join(compiledRoot, name), path.join(target, name));
  const monitorSource = path.resolve(compiledRoot, "..", "collectors", "process-start-monitor.ps1");
  if (!fs.existsSync(monitorSource)) throw new PortableBaseMvpError("Release-contained process monitor source is missing.", "collector_monitor_missing");
  fs.copyFileSync(monitorSource, path.join(target, "process-start-monitor.ps1"));
  writeText(path.join(target, "restricted-user-collector.cjs"), portableCollectorEntryScript());
  writeText(path.join(target, "privileged-observer.cjs"), privilegedObserverEntryScript());
}

function collectorManifestRecord(packageRoot: string, relative: string): Record<string, unknown> {
  const full = path.join(packageRoot, ...relative.split("/"));
  return { path: relative, size: fs.statSync(full).size, sha256: sha256File(full), required: true };
}

function portableCollectorEntryScript(): string {
  let source=`const crypto=require('node:crypto');const fs=require('node:fs');const path=require('node:path');const{spawn,spawnSync}=require('node:child_process');const{readObserverJsonFile}=require('./restricted-user-privileged-observer.js');
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');const root=fs.realpathSync(path.resolve(__dirname,'..'));const runtime=fs.realpathSync(process.execPath);const collector=fs.realpathSync(__filename);const monitor=fs.realpathSync(path.join(__dirname,'process-start-monitor.ps1'));const manifest=JSON.parse(fs.readFileSync(path.join(root,'release-manifest.json'),'utf8'));const record=r=>manifest.files.find(f=>f.path===r);const fail=(c,m)=>{const e=new Error(m);e.code=c;throw e};
if(!runtime.startsWith(root+path.sep)||!collector.startsWith(root+path.sep)||!monitor.startsWith(root+path.sep))fail('RELEASE_CONTAINED_COLLECTOR_REQUIRED','Runtime, collector, and monitor must share the verified release root.');if(runtime.toLowerCase()!==path.join(root,'runtime','node.exe').toLowerCase())fail('RELEASE_RUNTIME_REQUIRED','Bundled runtime required.');for(const [rel,file] of [['collectors/restricted-user-collector.cjs',collector],['collectors/process-start-monitor.ps1',monitor]]){const r=record(rel);if(!r||r.sha256!==sha(file)||r.size!==fs.statSync(file).size)fail('RELEASE_COLLECTOR_DIGEST_MISMATCH',rel);}
const action=process.argv[2],argument=process.argv[3],invocation=process.env.SERA_PROOF_INVOCATION||'NON_PROMOTABLE_DEVELOPMENT_SMOKE';if(invocation==='REAL_RESTRICTED_USER_PROOF'&&!argument)fail('EVIDENCE_ROOT_REQUIRED','Evidence root required.');const obs=require('./restricted-user-observations.js'),bindings=require('./restricted-user-production-bindings.js'),restart=require('./restricted-user-restart.js');
function identity(prepPath){const prep=JSON.parse(fs.readFileSync(prepPath,'utf8')),preparedMode=prep.invocationMode||(action==='smoke'?'NON_PROMOTABLE_DEVELOPMENT_SMOKE':null);if(!preparedMode)fail('PREPARATION_INVOCATION_MODE_REQUIRED');if(preparedMode!==invocation)fail(preparedMode==='NON_PROMOTABLE_PRODUCTION_ROUNDTRIP_TEST'?'PRODUCTION_TEST_MODE_RELABELLED':'PREPARATION_COLLECTOR_MODE_MISMATCH');if(collector.toLowerCase().startsWith(String(prep.repository.canonicalPath).toLowerCase()+path.sep))fail('REPOSITORY_COLLECTOR_PROHIBITED','Repository collector cannot produce proof.');const expected=prep.collectors.find(c=>path.resolve(c.path).toLowerCase()===collector.toLowerCase());if(!expected||expected.sha256!==sha(collector))fail('PREPARATION_COLLECTOR_MISMATCH','Preparation did not pin executing collector.');return prep;}
function provenance(prepPath){const prep=identity(prepPath),evidence=path.dirname(path.resolve(prepPath)),p={schemaVersion:'sera.release-collector-provenance.v1',invocation,releaseRoot:root,runtimePath:runtime,collectorPath:collector,monitorPath:monitor,runtimePid:process.pid,collectorPid:process.pid,parentPid:process.ppid,args:process.argv.slice(2),startedAt:new Date().toISOString(),proofSid:prep.expectedProofSid,sessionId:prep.sessionId,nonce:prep.nonce,collectorSize:fs.statSync(collector).size,collectorSha256:sha(collector),monitorSize:fs.statSync(monitor).size,monitorSha256:sha(monitor),runtimeSize:fs.statSync(runtime).size,runtimeSha256:sha(runtime),releaseIdentity:manifest.version};fs.mkdirSync(path.join(evidence,'pre'),{recursive:true});fs.writeFileSync(path.join(evidence,'pre','collector-provenance.json'),JSON.stringify(p,null,2)+'\\n',{flag:'wx'});return{prep,evidence,p};}
function verifyObserverReady(prepPath){const x=provenance(prepPath),observerRoot=path.resolve(String(x.prep.observerRoot||''));if(!observerRoot||!fs.existsSync(observerRoot))fail('PRIVILEGED_OBSERVER_READY_REQUIRED','Observer root missing.');const startPath=path.join(observerRoot,'observer-start.json'),readyPath=path.join(observerRoot,'monitor-ready.json');if(!fs.existsSync(startPath)||!fs.existsSync(readyPath))fail('PRIVILEGED_OBSERVER_READY_REQUIRED','Observer ready evidence missing.');const start=readObserverJsonFile(startPath),ready=readObserverJsonFile(readyPath);if(start.invocation!=='REAL_RESTRICTED_USER_PROOF')fail('OBSERVER_DEVELOPMENT_SMOKE_NON_PROMOTABLE','Development smoke cannot start real collection.');if(start.sessionId!==x.prep.sessionId||ready.sessionId!==x.prep.sessionId)fail('OBSERVER_SESSION_MISMATCH');if(start.nonce!==x.prep.nonce||ready.nonce!==x.prep.nonce)fail('OBSERVER_NONCE_MISMATCH');if(start.subjectSid!==x.prep.expectedProofSid||ready.proofSid!==x.prep.expectedProofSid)fail('OBSERVER_SUBJECT_SID_MISMATCH');if(start.releaseIdentity!==manifest.version)fail('OBSERVER_RELEASE_MISMATCH');if(start.elevated!==true||start.integrityLevel!=='High'||ready.liveSubscription!==true||ready.sourceEventType!=='Win32_ProcessStartTrace')fail('OBSERVER_NOT_LIVE');if(fs.existsSync(path.join(observerRoot,'monitor-complete.json')))fail('OBSERVER_WINDOW_NOT_ACTIVE');if(Date.now()-Date.parse(start.observerReadyAt)>15*60*1000)fail('OBSERVER_READY_STALE');const binding={schemaVersion:'sera.subject-observer-ready-binding.v1',sessionId:x.prep.sessionId,nonce:x.prep.nonce,subjectSid:x.prep.expectedProofSid,observerSid:start.observerSid,hostProfileId:start.hostProfileId,releaseIdentity:start.releaseIdentity,runtimeSha256:start.runtimeSha256,observerSha256:start.observerSha256,monitorSha256:start.monitorSha256,observerReadyAt:start.observerReadyAt,sourceEventType:ready.sourceEventType,liveSubscription:ready.liveSubscription,observerRoot};fs.writeFileSync(path.join(x.evidence,'pre','observer-ready-binding.json'),JSON.stringify(binding,null,2)+'\\n',{flag:'wx'});fs.writeFileSync(path.join(x.evidence,'pre','workflow-start.json'),JSON.stringify({workflowStartedAt:new Date().toISOString(),sessionId:x.prep.sessionId,nonce:x.prep.nonce,observerReadyAt:start.observerReadyAt})+'\\n',{flag:'wx'});return x;}
function completeSubject(prepPath){const prep=identity(prepPath),evidence=path.dirname(path.resolve(prepPath));fs.writeFileSync(path.join(evidence,'pre','workflow-complete.json'),JSON.stringify({workflowCompletedAt:new Date().toISOString(),sessionId:prep.sessionId,nonce:prep.nonce,subjectDeclaredOnly:true})+'\\n',{flag:'wx'});}
const ps='C:\\\\Windows\\\\System32\\\\WindowsPowerShell\\\\v1.0\\\\powershell.exe';const sleep=ms=>Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,ms);function start(prepPath){const x=provenance(prepPath),dir=path.join(x.evidence,'monitor');fs.mkdirSync(dir,{recursive:true});const monitorArgs=['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',monitor,'-EvidenceRoot',dir,'-SessionId',x.prep.sessionId,'-Nonce',x.prep.nonce,'-ProofSid',x.prep.expectedProofSid,'-CollectorSha256',sha(collector)];if(invocation==='NON_PROMOTABLE_DEVELOPMENT_SMOKE')monitorArgs.push('-AllowPollingFallback');const child=spawn(ps,monitorArgs,{shell:false,detached:true,windowsHide:true,stdio:invocation==='NON_PROMOTABLE_DEVELOPMENT_SMOKE'?['ignore','ignore','inherit']:'ignore',env:{SystemRoot:process.env.SystemRoot,WINDIR:process.env.WINDIR,TEMP:process.env.TEMP,TMP:process.env.TMP,PSModulePath:process.env.PSModulePath,ComSpec:process.env.ComSpec}});child.unref();const ready=path.join(dir,'monitor-ready.json'),until=Date.now()+10000;while(!fs.existsSync(ready)&&Date.now()<until)sleep(50);if(!fs.existsSync(ready))fail('LIVE_MONITOR_HANDSHAKE_MISSING','Live monitor did not become ready.');fs.writeFileSync(path.join(x.evidence,'pre','workflow-start.json'),JSON.stringify({workflowStartedAt:new Date().toISOString(),sessionId:x.prep.sessionId,nonce:x.prep.nonce,watcherPid:child.pid})+'\\n',{flag:'wx'});return x;}
function stop(prepPath){const prep=identity(prepPath),evidence=path.dirname(path.resolve(prepPath)),dir=path.join(evidence,'monitor');fs.writeFileSync(path.join(evidence,'pre','workflow-complete.json'),JSON.stringify({workflowCompletedAt:new Date().toISOString(),sessionId:prep.sessionId,nonce:prep.nonce})+'\\n',{flag:'wx'});fs.writeFileSync(path.join(dir,'monitor-stop.request'),'stop\\n',{flag:'wx'});const complete=path.join(dir,'monitor-complete.json'),until=Date.now()+10000;while(!fs.existsSync(complete)&&Date.now()<until)sleep(50);if(!fs.existsSync(complete))fail('LIVE_MONITOR_COMPLETION_MISSING','Live monitor did not flush and complete.');const ready=JSON.parse(fs.readFileSync(path.join(dir,'monitor-ready.json'),'utf8')),done=JSON.parse(fs.readFileSync(complete,'utf8')),workflowStart=JSON.parse(fs.readFileSync(path.join(evidence,'pre','workflow-start.json'),'utf8')),workflowEnd=JSON.parse(fs.readFileSync(path.join(evidence,'pre','workflow-complete.json'),'utf8')),events=fs.existsSync(path.join(dir,'monitor-events.ndjson'))?fs.readFileSync(path.join(dir,'monitor-events.ndjson'),'utf8').split(/\\r?\\n/).filter(Boolean).map(JSON.parse):[];const normalized={sessionId:prep.sessionId,nonce:prep.nonce,proofSid:prep.expectedProofSid,liveSubscription:ready.liveSubscription===true,sourceEventType:ready.sourceEventType,readyHandshake:ready.ready===true,flushed:done.flushed===true,completeness:done.complete&&done.flushed&&!done.interrupted?'COMPLETE':'INTERRUPTED',startedAt:done.monitorStartedAt,stoppedAt:done.monitorStoppedAt,workflowStartedAt:workflowStart.workflowStartedAt,workflowCompletedAt:workflowEnd.workflowCompletedAt,watcherPid:done.watcherPid,collectorSha256:sha(collector),events:events.map(e=>({timestamp:e.eventTimestamp,pid:e.pid,parentPid:e.parentPid,executableName:e.processName,executablePath:e.enrichment&&e.enrichment.executablePath,commandLine:e.enrichment&&e.enrichment.commandLine,owner:e.enrichment&&e.enrichment.owner,ownerSid:e.enrichment&&e.enrichment.ownerSid,sessionId:e.enrichment&&e.enrichment.sessionId,sourceEventType:e.sourceEventType,enrichmentStatus:e.enrichmentStatus,original:e.original}))};fs.writeFileSync(path.join(evidence,'pre','process-events.json'),JSON.stringify(normalized,null,2)+'\\n');return normalized;}
(async()=>{if(action==='collect-pre-restart'){if(invocation==='REAL_RESTRICTED_USER_PROOF')verifyObserverReady(argument);else fail('OBSERVER_DEVELOPMENT_SMOKE_NON_PROMOTABLE','Repository delegation cannot collect promotable evidence.');console.log(JSON.stringify(await obs.collectPreRestart(argument)));return;}if(action==='capture'){const stage=process.argv[3],prep=process.argv[4];const r=obs.captureStage(stage,prep);if(stage==='evidence-history')completeSubject(prep);console.log(JSON.stringify(r));return;}if(action==='collect-post-restart'){console.log(JSON.stringify(obs.collectPostRestart(argument)));return;}if(action==='smoke'){const evidence=path.resolve(argument),prepPath=path.join(evidence,'preparation-manifest.json');process.env.SERA_PROOF_INVOCATION='NON_PROMOTABLE_DEVELOPMENT_SMOKE';const x=start(prepPath);const child=spawnSync(runtime,['-e','setTimeout(()=>process.stdout.write(\\"harmless-release-child\\"),1000)'],{shell:false,encoding:'utf8',cwd:root,env:{SystemRoot:process.env.SystemRoot,WINDIR:process.env.WINDIR,TEMP:process.env.TEMP,TMP:process.env.TMP,PATH:path.dirname(runtime)}});sleep(500);const observed=stop(prepPath);console.log(JSON.stringify({ok:child.status===0,status:'NON_PROMOTABLE_DEVELOPMENT_SMOKE',claimsGranted:[],collectorPath:collector,collectorSha256:sha(collector),monitorPath:monitor,monitorSha256:sha(monitor),eventCount:observed.events.length}));return;}fail('UNKNOWN_COLLECTOR_ACTION','Unknown collector action.');})().catch(e=>{console.error(JSON.stringify({ok:false,status:'BLOCKED',reasonCode:e.code||e.message,claimsGranted:[]}));process.exitCode=1;});`;
  source=source.replace("console.log(JSON.stringify(await obs.collectPreRestart(argument)));return;","const result=await obs.collectPreRestart(argument),prep=identity(argument),evidence=path.dirname(path.resolve(argument)),boot=JSON.parse(fs.readFileSync(path.join(evidence,'pre','boot.json'),'utf8'));bindings.emitSubjectBindingRecords(evidence,prep,'PRE_RESTART',boot,{runtimePath:runtime,processId:process.pid,parentProcessId:process.ppid,proofSid:prep.expectedProofSid,windowsSessionId:process.env.SESSIONNAME});console.log(JSON.stringify(result));return;");
  source=source.replace("console.log(JSON.stringify(obs.collectPostRestart(argument)));return;","const prep=identity(argument),evidence=path.dirname(path.resolve(argument)),before=JSON.parse(fs.readFileSync(path.join(evidence,'pre','boot.json'),'utf8')),after=restart.collectBootObservation({hostProfileId:prep.hostProfileId,installationId:prep.installationIdentity,sessionId:prep.sessionId,nonce:prep.nonce,processId:process.pid});restart.compareBootObservations(before,after);bindings.emitSubjectBindingRecords(evidence,prep,'POST_RESTART',after,{runtimePath:runtime,processId:process.pid,parentProcessId:process.ppid,proofSid:prep.expectedProofSid,windowsSessionId:process.env.SESSIONNAME});console.log(JSON.stringify(obs.collectPostRestart(argument)));return;");
  source=source.replace("(async()=>{if(action==='collect-pre-restart'){","(async()=>{if(invocation==='NON_PROMOTABLE_PRODUCTION_ROUNDTRIP_TEST'&&(action==='collect-pre-restart'||action==='collect-post-restart')){const prep=identity(argument),stage=action==='collect-pre-restart'?'PRE_RESTART':'POST_RESTART';console.log(JSON.stringify(bindings.runPackagedRoundTripSubject(argument,stage,{runtimePath:runtime,collectorPath:collector})));return;}if(action==='collect-pre-restart'){");
  return source;
}

function privilegedObserverEntryScript(): string {
  const script=`const crypto=require('node:crypto');const fs=require('node:fs');const path=require('node:path');const{spawnSync,spawn}=require('node:child_process');const{parseObserverJsonText,readObserverJsonFile,isOwnedObserverChild}=require('./restricted-user-privileged-observer.js'),bindings=require('./restricted-user-production-bindings.js'),restart=require('./restricted-user-restart.js');
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex'),fail=(code,detail={})=>{console.error(JSON.stringify({ok:false,status:'BLOCKED',reasonCode:code,detail,claimsGranted:[]}));process.exit(1)};
const root=fs.realpathSync(path.resolve(__dirname,'..')),runtime=fs.realpathSync(process.execPath),observer=fs.realpathSync(__filename),manifest=readObserverJsonFile(path.join(root,'release-manifest.json')),value=n=>{const i=process.argv.indexOf(n);return i<0?null:process.argv[i+1]},monitor=fs.realpathSync(value('--monitor-script')||path.join(__dirname,'process-start-monitor.ps1')),prepPath=value('--preparation'),observerRoot=value('--observer-root'),sessionId=value('--session-id'),subjectSid=value('--subject-sid');
const rec=r=>manifest.files.find(f=>f.path===r);for(const [rel,file] of [['runtime/node.exe',runtime],['collectors/privileged-observer.cjs',observer],['collectors/process-start-monitor.ps1',monitor]]){const x=rec(rel);if(!file.startsWith(root+path.sep)||!x||x.sha256!==sha(file)||x.size!==fs.statSync(file).size)fail('OBSERVER_RELEASE_BINDING_INVALID',{rel});}if(!prepPath||!observerRoot||!sessionId||!subjectSid||!value('--nonce-file'))fail('OBSERVER_ARGUMENTS_REQUIRED');
const prep=readObserverJsonFile(prepPath);if(prep.sessionId!==sessionId||prep.expectedProofSid!==subjectSid)fail('OBSERVER_PREPARATION_MISMATCH');const nonceFile=value('--nonce-file');let nonceRecord;try{const raw=fs.readFileSync(nonceFile,'utf8');try{nonceRecord=parseObserverJsonText(raw).nonce}catch{nonceRecord=raw.trim()}}catch{fail('OBSERVER_NONCE_MISMATCH')};const nonce=prep.nonce;if(!nonce||nonceRecord!==nonce)fail('OBSERVER_NONCE_MISMATCH');
const who=spawnSync('C:\\\\Windows\\\\System32\\\\whoami.exe',['/user','/fo','csv','/nh'],{shell:false,encoding:'utf8',windowsHide:true}),groups=spawnSync('C:\\\\Windows\\\\System32\\\\whoami.exe',['/groups','/fo','csv','/nh'],{shell:false,encoding:'utf8',windowsHide:true}),observerSid=(String(who.stdout).match(/S-1-[0-9-]+/)||[])[0],admin=/S-1-5-32-544/.test(String(groups.stdout))&&/Enabled group/i.test(String(groups.stdout));if(!admin)fail('PRIVILEGED_OBSERVER_REQUIRED',{observerSid,tokenElevationState:'not-elevated',exitCode:groups.status,stderr:String(groups.stderr||who.stderr||''),subscriptionState:'NOT_ATTEMPTED',handshakeState:'NOT_CREATED',timestamp:new Date().toISOString()});if(observerSid===subjectSid)fail('OBSERVER_SUBJECT_SID_COLLISION');
fs.mkdirSync(observerRoot,{recursive:true});const start={schemaVersion:'sera.privileged-observer-start.v1',invocation:process.argv[2]==='smoke'?'NON_PROMOTABLE_DEVELOPMENT_SMOKE':'REAL_RESTRICTED_USER_PROOF',observerProfile:'privileged-observer-sidecar-v1',observerUsername:process.env.USERNAME,observerSid,elevated:true,integrityLevel:'High',enabledGroupSids:['S-1-5-32-544'],windowsSessionId:process.env.SESSIONNAME,pid:process.pid,parentPid:process.ppid,runtimePath:runtime,runtimeSha256:sha(runtime),observerPath:observer,observerSha256:sha(observer),monitorPath:monitor,monitorSha256:sha(monitor),launchTimestamp:new Date().toISOString(),sessionId,nonce,subjectSid,hostProfileId:prep.hostProfileId,releaseIdentity:manifest.version,preparationManifestSha256:sha(prepPath),governanceDecisionSha256:prep.governanceDecisionDigest,releaseZipSha256:prep.release?.zipSha256,extractedTreeSha256:prep.release?.extractedTreeDigest,releaseManifestSha256:prep.release?.releaseManifestDigest,installationIdentity:prep.installationIdentity,bootStage:prep.bootStage,ready:false,subscriptionState:'STARTING'};fs.writeFileSync(path.join(observerRoot,'observer-start.json'),JSON.stringify(start,null,2)+'\\n',{flag:'wx'});
const ps='C:\\\\Windows\\\\System32\\\\WindowsPowerShell\\\\v1.0\\\\powershell.exe',args=['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',monitor,'-EvidenceRoot',observerRoot,'-SessionId',sessionId,'-Nonce',nonce,'-ProofSid',subjectSid,'-CollectorSha256',sha(observer)],stderr=path.join(observerRoot,'monitor-stderr.txt'),err=fs.openSync(stderr,'wx'),child=spawn(ps,args,{shell:false,windowsHide:true,detached:false,stdio:['ignore','ignore',err],env:{SystemRoot:process.env.SystemRoot,WINDIR:process.env.WINDIR,TEMP:process.env.TEMP,TMP:process.env.TMP,PSModulePath:process.env.PSModulePath}}),ownedMonitorPid=child.pid,sleep=ms=>Atomics.wait(new Int32Array(new SharedArrayBuffer(4)),0,0,ms),readyPath=path.join(observerRoot,'monitor-ready.json'),failurePath=path.join(observerRoot,'monitor-failure.json');
const terminateOwnedMonitor=reason=>{if(!isOwnedObserverChild(ownedMonitorPid,child.pid))return false;if(!child.killed){child.kill();sleep(500)}const artifact={schemaVersion:'sera.privileged-observer-owned-child-failure.v1',reason,observerPid:process.pid,ownedMonitorPid,terminationRequested:child.killed===true,unrelatedProcessTerminationAttempted:false,timestamp:new Date().toISOString()};fs.writeFileSync(path.join(observerRoot,'observer-child-cleanup.json'),JSON.stringify(artifact,null,2)+'\\n');return true};
const until=Date.now()+10000;while(!fs.existsSync(readyPath)&&!fs.existsSync(failurePath)&&Date.now()<until)sleep(50);fs.closeSync(err);if(!fs.existsSync(readyPath)){if(!fs.existsSync(failurePath))fs.writeFileSync(failurePath,JSON.stringify({schemaVersion:'sera.privileged-observer-failure.v1',classification:'TIMEOUT',exceptionType:null,hresult:null,stderr:fs.readFileSync(stderr,'utf8'),exitCode:child.exitCode,subscriptionState:'UNKNOWN',handshakeState:'NOT_CREATED',timestamp:new Date().toISOString()},null,2)+'\\n');terminateOwnedMonitor('OBSERVER_SUBSCRIPTION_FAILED');fail('OBSERVER_SUBSCRIPTION_FAILED',readObserverJsonFile(failurePath))}
let ready;try{ready=readObserverJsonFile(readyPath)}catch(error){terminateOwnedMonitor('OBSERVER_READY_JSON_INVALID');fs.writeFileSync(failurePath,JSON.stringify({schemaVersion:'sera.privileged-observer-failure.v1',classification:'READY_JSON_INVALID',exceptionType:error.constructor.name,message:error.message,ownedMonitorPid,subscriptionState:'UNKNOWN',handshakeState:'MALFORMED',timestamp:new Date().toISOString()},null,2)+'\\n');fail('OBSERVER_READY_JSON_INVALID')}
Object.assign(start,{ready:true,liveSubscription:ready.liveSubscription,sourceEventType:ready.sourceEventType,subscriptionState:'SUBSCRIBED',observerReadyAt:new Date().toISOString(),monitorPid:child.pid});fs.writeFileSync(path.join(observerRoot,'observer-start.json'),JSON.stringify(start,null,2)+'\\n');if(process.argv[2]==='smoke'){const workflowStartedAt=new Date().toISOString(),probe=spawnSync(runtime,['-e','setTimeout(()=>process.stdout.write(\\"SERA_OBSERVER_SMOKE_PROBE\\"),2000)'],{shell:false,encoding:'utf8',windowsHide:true,cwd:root,env:{SystemRoot:process.env.SystemRoot,WINDIR:process.env.WINDIR,TEMP:process.env.TEMP,TMP:process.env.TMP,PATH:path.dirname(runtime)}}),workflowCompletedAt=new Date().toISOString();fs.writeFileSync(path.join(observerRoot,'workflow-window.json'),JSON.stringify({schemaVersion:'sera.observer-smoke-window.v1',invocation:'NON_PROMOTABLE_DEVELOPMENT_SMOKE',sessionId,nonce,subjectSid,workflowStartedAt,workflowCompletedAt,probePid:probe.pid,probeExitCode:probe.status},null,2)+'\\n',{flag:'wx'});fs.writeFileSync(path.join(observerRoot,'monitor-stop.request'),'stop\\n',{flag:'wx'});const completePath=path.join(observerRoot,'monitor-complete.json'),stopUntil=Date.now()+10000;while(!fs.existsSync(completePath)&&Date.now()<stopUntil)sleep(50);if(!fs.existsSync(completePath)){terminateOwnedMonitor('OBSERVER_FLUSH_TIMEOUT');fail('OBSERVER_FLUSH_TIMEOUT')}const complete=readObserverJsonFile(completePath),eventPath=path.join(observerRoot,'monitor-events.ndjson'),events=fs.existsSync(eventPath)?fs.readFileSync(eventPath,'utf8').split(/\\r?\\n/).filter(Boolean).map(parseObserverJsonText):[],probeEvent=events.find(e=>e.pid===probe.pid);if(!probeEvent)fail('OBSERVER_SMOKE_CHILD_MISSING');if(probeEvent.enrichmentStatus!=='OBSERVED_AND_ENRICHED')fail('OBSERVER_SMOKE_ENRICHMENT_INCOMPLETE',{enrichmentStatus:probeEvent.enrichmentStatus});const result={schemaVersion:'sera.privileged-observer-smoke-result.v1',ok:true,status:'NON_PROMOTABLE_DEVELOPMENT_SMOKE',smokeIdentity:'SERA_OBSERVER_SMOKE_V5_1',claimsGranted:[],sessionId,nonce,subjectSid,observerSid,runtimePath:runtime,runtimeSha256:sha(runtime),observerPath:observer,observerSha256:sha(observer),monitorPath:monitor,monitorSha256:sha(monitor),sourceEventType:ready.sourceEventType,liveSubscription:ready.liveSubscription===true,ready:true,observerReadyAt:start.observerReadyAt,workflowStartedAt,workflowCompletedAt,observerStoppedAt:complete.monitorStoppedAt,flushed:complete.flushed===true,unsubscribed:complete.unsubscribed===true,probePid:probe.pid,probeEvent};fs.writeFileSync(path.join(observerRoot,'smoke-result.json'),JSON.stringify(result,null,2)+'\\n',{flag:'wx'});console.log(JSON.stringify(result));return;}child.unref();console.log(JSON.stringify({ok:true,status:'OBSERVER_READY',pid:child.pid,observerRoot,claimsGranted:[]}));`;
  let emitted=script.replace("fs.writeFileSync(path.join(observerRoot,'observer-start.json'),JSON.stringify(start,null,2)+'\\n');if(process.argv[2]==='smoke')","fs.writeFileSync(path.join(observerRoot,'observer-start.json'),JSON.stringify(start,null,2)+'\\n');if(process.argv[2]!=='smoke'){const stage=value('--stage')||'PRE_RESTART',installation=bindings.readVerifiedInstallationIdentity(prep),boot=restart.collectBootObservation({hostProfileId:prep.hostProfileId,installationId:installation.installationId,sessionId,nonce,processId:process.pid});bindings.emitObserverBootStage(observerRoot,prep,stage,boot,start);}if(process.argv[2]==='smoke')");
  emitted=emitted.replace("const who=spawnSync","if(prep.invocationMode==='NON_PROMOTABLE_PRODUCTION_ROUNDTRIP_TEST'){if(path.resolve(observerRoot)!==path.resolve(prep.observerRoot))fail('OBSERVER_ROOT_MISMATCH');const stage=value('--stage')||'PRE_RESTART',result=bindings.runPackagedRoundTripObserver(prepPath,stage,{runtimePath:runtime,observerPath:observer,monitorPath:monitor});console.log(JSON.stringify(result));process.exit(0);}const who=spawnSync");
  return emitted.replace("child.unref();console.log(JSON.stringify({ok:true,status:'OBSERVER_READY',pid:child.pid,observerRoot,claimsGranted:[]}));",`console.log(JSON.stringify({ok:true,status:'OBSERVER_READY',pid:child.pid,observerRoot,claimsGranted:[]}));const subjectRoot=path.dirname(prepPath),subjectCompletePath=path.join(subjectRoot,'pre','workflow-complete.json'),workflowUntil=Date.now()+8*60*60*1000;while(!fs.existsSync(subjectCompletePath)&&Date.now()<workflowUntil)sleep(250);if(!fs.existsSync(subjectCompletePath)){terminateOwnedMonitor('SUBJECT_WORKFLOW_COMPLETION_TIMEOUT');fail('SUBJECT_WORKFLOW_COMPLETION_TIMEOUT')}const subjectComplete=readObserverJsonFile(subjectCompletePath);if(subjectComplete.sessionId!==sessionId||subjectComplete.nonce!==nonce)fail('SUBJECT_WORKFLOW_COMPLETION_MISMATCH');fs.writeFileSync(path.join(observerRoot,'monitor-stop.request'),'stop\\n',{flag:'wx'});const finalPath=path.join(observerRoot,'monitor-complete.json'),finalUntil=Date.now()+10000;while(!fs.existsSync(finalPath)&&Date.now()<finalUntil)sleep(50);if(!fs.existsSync(finalPath)){terminateOwnedMonitor('OBSERVER_FLUSH_TIMEOUT');fail('OBSERVER_FLUSH_TIMEOUT')}const finalRecord=readObserverJsonFile(finalPath);if(finalRecord.flushed!==true||finalRecord.unsubscribed!==true)fail('OBSERVER_FINALIZATION_INCOMPLETE');console.log(JSON.stringify({ok:true,status:'OBSERVER_FINALIZED',sessionId,nonce,observerStoppedAt:finalRecord.monitorStoppedAt,claimsGranted:[]}));`);
}
