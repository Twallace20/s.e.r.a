...
export function auditPrivilegedObserver(i:PrivilegedObserverAuditInput):string[]{
  ...
  const runtimeReal=real(i.runtimePath).toLowerCase(),runtimeDigest=digest(i.runtimePath).toLowerCase(),nodeEvents=events.filter(e=>/^node\.exe$/i.test(e.executableName)),identifiedNodeEvents=nodeEvents.filter(e=>typeof e.executablePath==="string"&&e.executablePath.length>0&&typeof e.executableSha256==="string"&&/^[a-f0-9]{64}$/i.test(e.executableSha256)),goodNode=identifiedNodeEvents.some(e=>real(e.executablePath).toLowerCase()===runtimeReal&&String(e.executableSha256).toLowerCase()===runtimeDigest),badNode=identifiedNodeEvents.some(e=>real(e.executablePath).toLowerCase()!==runtimeReal||String(e.executableSha256).toLowerCase()!==runtimeDigest);
  fail("OBSERVER_WRONG_NODE_IDENTITY",goodNode&&!badNode&&!events.some(e=>devTools.test(e.executableName)&&!/^git|n(pm|px)/i.test(e.executableName)));
...
export function observerAclPlan(releaseRoot:string,subjectRoot:string,observerRoot:string,subjectSid:string):Record<string,unknown>{
  return{status:"PLAN_ONLY_NOT_APPLIED",apply:[`icacls "${releaseRoot}" /grant *${subjectSid}:(OI)(CI)RX`,`icacls "${subjectRoot}" /grant *${subjectSid}:(OI)(CI)M`,`icacls "${observerRoot}" /inheritance:r`,`icacls "${observerRoot}" /grant:r *S-1-5-32-544:(OI)(CI)F`,`icacls "${observerRoot}" /grant *${subjectSid}:(OI)(CI)RX`],rollback:[`icacls "${observerRoot}" /remove:g *${subjectSid}`,`icacls "${observerRoot}" /inheritance:e`,`icacls "${subjectRoot}" /remove:g *${subjectSid}`,`icacls "${releaseRoot}" /remove:g *${subjectSid}`],applied:false};
}
...
export function verifyPrivilegedObserverEvidence(subjectRootInput:string,observerRootInput:string):Record<string,unknown>{
  ...
  const preparation=read(subjectRoot,"preparation-manifest.json"),subject=read(subjectRoot,"pre/account-token.json"),workflowStart=read(subjectRoot,"pre/workflow-start.json"),workflowComplete=read(subjectRoot,"pre/workflow-complete.json"),observerStart=read(observerRoot,"observer-start.json"),observerCompleteRaw=read(observerRoot,"monitor-complete.json"),observerComplete={...observerCompleteRaw,observerStoppedAt:observerCompleteRaw.observerStoppedAt??observerCompleteRaw.monitorStoppedAt},spool=read(observerRoot,"spool-protection.json"),releaseRoot=String(preparation.release?.expectedExtractionRoot??""),releaseManifest=read(releaseRoot,"release-manifest.json"),eventsFile=path.join(observerRoot,"monitor-events.ndjson"),events=fs.existsSync(eventsFile)?fs.readFileSync(eventsFile,"utf8").split(/\r?\n/).filter(Boolean).map(line=>{const e=JSON.parse(line);return{...e,timestamp:e.eventTimestamp,executableName:e.processName,executablePath:e.enrichment?.executablePath,executableSha256:e.enrichment?.executableSha256,ownerSid:e.enrichment?.ownerSid,sessionId:e.enrichment?.sessionId}}):[];
