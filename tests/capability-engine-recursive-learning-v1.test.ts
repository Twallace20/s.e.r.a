...
describe("Capability Engine and Recursive Learning v1", () => {
  ...
  const proofCases: Array<[string, () => unknown]> = [
    ...
    ["rollback restores target lifecycle to PROMOTED", () => firstCapabilityProof.rollbackRestoredTargetPromoted],
    ["rollback marks replaced version ROLLED_BACK", () => firstCapabilityProof.rollbackMarkedCurrentRolledBack],
    ["rollback preserves Control Plane pointer authority", () => firstCapabilityProof.rollbackPointerAuthorityPreserved],
    ["rollback restores catalog to active PROMOTED state", () => firstCapabilityProof.rollbackCatalogRestoredPromoted],
    ["rollback records exact current and target digests", () => firstCapabilityProof.rollbackRecordExact],
