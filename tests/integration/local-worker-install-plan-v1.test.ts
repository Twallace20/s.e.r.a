...
import { createDefaultLocalWorkerInstallPlanV1, inspectLocalWorkerInstallPlanV1 } from "../../scripts/lib/local-worker-install-plan-v1.mjs";

function writeFixture(rootDir: string) {
  const files = new Map<string, string>([
    ["docs/phases/PHASE_62_LOCAL_WORKER_INSTALL_PLAN_V1.md", "# Phase 62 Local Worker Install Plan v1"],
    ["apps/operator-console/src/local-worker-install-plan.ts", "export const localWorkerInstallPlanPacket = { phase: { label: 'Phase 62' }, installPlanSummary: { owner: 'Tyler Wallace', installPlanApproved: false }, installPlanRequirements: [], evidenceRequirements: [], boundaries: { workerInstallAllowed: false } };"],
    ["scripts/lib/local-worker-install-plan-v1.mjs", "export const marker = true;"],
    ["scripts/run-local-worker-install-plan-v1.mjs", "console.log('Local worker install plan');"],
    ["tests/integration/local-worker-install-plan-v1.test.ts", "// fixture"],
    ["apps/operator-console/src/App.tsx", `localWorkerInstallPlanPacket.installPlanSummary.owner
...
Local Worker Install Plan`],
    ["package.json", `{"scripts": {"phase62:demo": "node scripts/run-local-worker-install-plan-v1.mjs", "phase62:verify": "npm run phase62:demo"}}`],
  ]);
...
describe("local-worker-install-plan-v1", () => {
  it("passes when the local worker install plan is declarative and app-bound", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "sera-local-worker-install-plan-cert-"));
    writeFixture(rootDir);

    const result = inspectLocalWorkerInstallPlanV1(createDefaultLocalWorkerInstallPlanV1(), { rootDir, writeArtifacts: false });

    expect(result.ok).toBe(true);
    expect(result.localWorkerInstallPlanStatus).toBe("ready");
    expect(result.installPlanOnly).toBe(true);
    expect(result.ownerReviewOnly).toBe(true);
    expect(result.declarativeOnly).toBe(true);
    expect(result.phase61UnlockProposalReady).toBe(true);
  ...
  it("writes local worker install plan reports without mutating source", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "sera-local-worker-install-plan-report-cert-"));
    writeFixture(rootDir);

    const result = inspectLocalWorkerInstallPlanV1(createDefaultLocalWorkerInstallPlanV1(), { rootDir, writeArtifacts: true });

    expect(result.ok).toBe(true);
    expect(fs.existsSync(path.join(rootDir, ".sera-local-worker-install-plan", "phase62-local-worker-install-plan-status.json"))).toBe(true);
    expect(fs.existsSync(path.join(rootDir, ".sera-local-worker-install-plan", "phase62-local-worker-install-plan-status.md"))).toBe(true);
    expect(fs.existsSync(path.join(rootDir, "apps/operator-console/src/local-worker-install-plan.ts"))).toBe(true);
  });

  it("blocks unsafe local worker install plan boundaries", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "sera-local-worker-install-plan-block-cert-"));
    writeFixture(rootDir);

    const config = createDefaultLocalWorkerInstallPlanV1();
    ...
    config.boundaries.installPlanPersistenceAllowed = true;
    ...
    const result = inspectLocalWorkerInstallPlanV1(config, { rootDir, writeArtifacts: false });
    ...
    expect(result.blockers).toContain("installPlanPersistenceAllowed must remain false");
  ...
  it("blocks unsafe declared paths", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "sera-local-worker-install-plan-path-cert-"));
    writeFixture(rootDir);
