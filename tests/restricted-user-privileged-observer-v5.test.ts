...
describe("privileged observer sidecar v5",()=>{
  ...
  it("accepts an unresolved transient Node start when a positively identified packaged runtime Node is present",()=>{const f=fixture();f.input.events.push({executableName:"node.exe",enrichmentStatus:"ENRICHMENT_PROCESS_EXITED"});expect(auditPrivilegedObserver(f.input)).not.toContain("OBSERVER_WRONG_NODE_IDENTITY")});
  it("accepts an enrichment-error Node start when a positively identified packaged runtime Node is present",()=>{const f=fixture();f.input.events.push({executableName:"node.exe",enrichmentStatus:"ENRICHMENT_ERROR"});expect(auditPrivilegedObserver(f.input)).not.toContain("OBSERVER_WRONG_NODE_IDENTITY")});
  it("fails closed when no positively identified packaged runtime Node exists",()=>{const f=fixture();f.input.events=[{executableName:"node.exe",enrichmentStatus:"ENRICHMENT_PROCESS_EXITED"}];expect(auditPrivilegedObserver(f.input)).toContain("OBSERVER_WRONG_NODE_IDENTITY")});
  it("fails closed for a positively identified wrong Node even when the packaged runtime was also observed",()=>{const f=fixture();f.input.events.push({executableName:"node.exe",executablePath:path.join(f.base,"wrong-node.exe"),executableSha256:"0".repeat(64)});expect(auditPrivilegedObserver(f.input)).toContain("OBSERVER_WRONG_NODE_IDENTITY")});
  it("requires the production monitor to emit executableSha256 for Node identity evidence",()=>{const script=fs.readFileSync(path.join(process.cwd(),"packages","portable-base-mvp","collectors","process-start-monitor.ps1"),"utf8");expect(script).toContain("executableSha256=$executableSha256");expect(script).toContain("Get-FileHash");expect(script).toContain("-Algorithm SHA256")});
  ...
  it("keeps observer evidence readable but non-writable by the restricted subject",()=>{
    const f=fixture(),prep=path.join(f.base,"preparation-manifest.json");
    f.input.preparation.observerRoot=path.join(f.base,"observer-spool");
    f.input.preparation.repository={canonicalPath:path.join(f.base,"repo")};
    fs.writeFileSync(prep,JSON.stringify(f.input.preparation));
    const plan:any=privilegedObserverPlan(prep);
    const apply=plan.aclPlan.apply as string[];
    const rollback=plan.aclPlan.rollback as string[];
    expect(apply.some(command=>command.includes("/grant *subject:(OI)(CI)RX"))).toBe(true);
    expect(apply.some(command=>command.includes("/deny *subject:"))).toBe(false);
    expect(rollback.some(command=>command.includes("/remove:d *subject"))).toBe(false);
    expect(rollback.some(command=>command.includes("/remove:g *subject"))).toBe(true);
  });
});
