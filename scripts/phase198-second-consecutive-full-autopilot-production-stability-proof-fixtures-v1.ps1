...
$FixturesPath = Join-Path $RepoRoot "tests\fixtures\phase198-full-autopilot\cases.json"
...
foreach ($Case in @($Fixtures.cases)) {
  $Temp = Join-Path $env:TEMP ("phase198-fixture-{0}.json" -f ([guid]::NewGuid().ToString('N')))
  ...
  if ([string]$Result.status -ne [string]$Case.expectStatus) { $Failures += "$($Case.name): expected=$($Case.expectStatus) actual=$($Result.status) reason=$($Result.reason)" }
}
if ($Failures.Count -gt 0) { throw "Phase198 fixture failures:`n$($Failures -join "`n")" }
Write-Host "PHASE198_SECOND_FULL_AUTOPILOT_FIXTURE_PROOF_PASS cases=$(@($Fixtures.cases).Count)"
