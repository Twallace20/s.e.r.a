...
function simulateAutopilotForE2ETest(p, command) {
  ...
  ensureDir(p.handoff);
  if (mode === "closed") {
    const file = path.join(p.handoff, `s.e.r.a_phase${command.phaseStart}_phase132_real_life_proof_test-19700101_000000-CLOSED_CLEANLY.md`);
    writeText(file, `# S.E.R.A. AutoOps Packet\n\nStatus: CLOSED_CLEANLY\nPhase: phase${command.phaseStart}-phase132-real-life-proof-test\n\n## Summary\n\nSimulated real-life phone autopilot proof generated by ${VERSION}.\n`);
    return { status: 0, stdout: `Simulated CLOSED_CLEANLY handoff: ${file}\n`, stderr: "" };
  }
  if (mode === "blocked") {
    const file = path.join(p.handoff, `s.e.r.a_phase${command.phaseStart}_phase132_real_life_proof_test-19700101_000000-BLOCKED.md`);
