...
$ActualPhaseSlug = [string]$Context.phaseSlug
if ($ActualPhaseSlug -ne $PhaseSlug) {
  Result "BLOCKED" "Wrong phaseSlug for Phase197 gate. expected=$PhaseSlug actual=$ActualPhaseSlug" $Context
  exit 0
}

if ([string]$Context.expectedZipFilename -ne $ExpectedZipFilename) {
  Result "BLOCKED" "Wrong expected ZIP filename for Phase197. expected=$ExpectedZipFilename actual=$($Context.expectedZipFilename)" $Context
  exit 0
}

if ($Context.manualRescueUsed -eq $true) {
  Result "BLOCKED" "Manual rescue was used. Phase197 cannot prove full autopilot if manual rescue was required." $Context
  exit 0
}
...
foreach ($Step in @('freshCommandJsonAccepted','savedChatGptTargetUsed','promptSubmitted','exactZipDownloaded','exactZipShaVerified')) {
  if ($Context.$Step -ne $true) {
    Result "BLOCKED" "Full autopilot pre-closeout step missing or false: $Step" $Context
    exit 0
  }
}
...
foreach ($Step in @('mergeSucceeded','pushMainSucceeded','pushTagSucceeded')) {
  if ($Context.$Step -ne $true) {
    Result "BLOCKED" "Required closeout step failed or did not run: $Step" $Context
    exit 0
  }
}
...
Result "CLOSED_CLEANLY" "All Phase197 full autopilot and remote truth gates passed." $Context
exit 0
